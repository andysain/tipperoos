...
- **Capture UI**: players sort all 20 teams into 7 fixed Table Bands via a single-card swipe/tap gesture against fixed Band targets (team name + 3-letter code per team — no crest, per `docs/DESIGN_SYSTEM.md`'s no-crest trademark constraint). Sorting is free-form — a Band may be temporarily over- or under-filled while sorting — with any Band that doesn't match its target size visually flagged and fixed via a tap-team → move-to-Band action before submission is allowed. No drag-and-drop reordering exists anywhere in this feature — deliberately avoided as the highest-risk mobile UI option for the build timeline. Every move persists immediately (not just on final submit), so the flow is safely resumable. See `docs/adr/0003-predict-the-table-shape.md` for the fuller rationale and rejected alternatives.
