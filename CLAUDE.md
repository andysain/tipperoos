...
- No chat, comments, public profiles, or social features beyond the leaderboard and the pick reveal.
...
- Bot players exist (`is_bot = true`), clearly labelled on the leaderboard (e.g. 🤖). **No bot is eligible for the season "winner" title** — bots are there for fun and intrigue, and the season winner is always a person. This reverses an earlier "bots are eligible" rule; see `docs/adr/0009-match-scoring-formula-and-title-eligibility.md`, which also records the evidence behind it (in the retired World Cup app the Median Bot led most of the season and won it, and the Random Bot finished a substantial last). **Bots are per-competition** — each competition has its own bot players, scoped by `players.competition_id`; the Median Bot in particular must derive from its own competition's human picks only. Bot picks are **written once and never re-rolled** — generated automatically on the existing fixture-sync cadence, so a bot's pick, once visible, never changes underneath a player. Three bot types carry forward from the old app (ported logic, not reinvented); the ELO bot is dropped (see _Explicitly out of scope_):
  - **Random Bot**: predicts a random plausible scoreline for each side, independently, per match.
  - **1-1 Bot**: always predicts 1–1.
  - **Median Bot**: predicts the rounded median of that match's human players' submitted picks, each side medianed independently (so its scoreline may be one nobody submitted) and rounded half-up. Generated only _after_ the match locks (not a blind guess) — it's a "wisdom of the crowd" reference pick, not a competitive prediction. If no human picked that match at all, it falls back to 1–1, which makes it identical to the 1-1 Bot for that match; accepted as a faithful port of the old app rather than a special case worth new rules (issue #35 D9). With bots now ineligible for the title, it functions as the leaderboard's **benchmark line**: beating the crowd's own consensus over a season is the one comparison in the app that says something about skill rather than luck.
- **Late joiners**: a player who signs up (via the private competition code) after gameweek 1 has begun. They **are eligible for the season "winner" title** — reversed 2026-08-16 from an earlier rule excluding them, on the grounds that joining late carries no advantage to protect against: a late joiner has strictly fewer scored gameweeks, so a cumulative points total already handicaps them without a rule doing it, and an exclusion rule bought nothing but an explanation. They **remain ineligible for the separate Predict the Table title**, where the asymmetry runs the other way — submitting a table after results exist is a real information advantage, which is also why a Late Joiner sits outside the Bold Call process in both directions. They **can submit Predict the Table at any time after joining, or skip it entirely** — both optional for them, unlike the mandatory pre-season capture for players who join before gameweek 1. Gameweeks before they joined score 0, with no special-case logic needed beyond "no picks exist for those matches" — the leaderboard's points-per-gameweek-played column (see _Scoring_) is what stops that reading as poor form rather than absence.
...
The app's landing route `/` **is** the pick board; there is no hub or dashboard in front of it. A newly signed-up player is given a prominent next-step prompt and a temporary `Next up` marker on the Predict the Table tab so the onboarding task is discoverable without adding a separate hub. See `docs/adr/0007-home-surface-and-pick-entry.md` for the full shape, its states, the entry mechanic, and the prototyped alternatives that were rejected.
...
- **Two slots, shown in kickoff order** — the earlier kickoff renders on top, the **Headline (marquee) breaking a kickoff tie**. The `gameweeks` slots keep their sourced meaning (`match_1` = top-ranked matchup, `match_2` = uniform random, ADR 0006) and are never reordered in the DB; the board only renders them by kickoff. The shown order is fixed per gameweek and never shuffles. Both open slots show their entry controls immediately — nothing needs opening first.
- **A settled slot** (filed, locked, live or finished) collapses to a dark plate carrying the scoreline and the player's own points. **Comparing against other players lives on the pick reveal (`/gameweek/[n]`), not here** — home shows a player their own pick and their own outcome only, which is also how the pre-lock visibility rule above is upheld by construction.
...
- Captured once, as part of onboarding; editable and re-submittable any number of times until the end of 31 August 2026 in Australia/Sydney, then locked — same lock-timing pattern as regular picks, not a single-shot submission. The server enforces the exclusive UTC cutoff `2026-08-31T14:00:00Z` using DB time. A Late Joiner, identified by joining after Gameweek 1's first kickoff, may submit at any time after joining, or skip it entirely.
...
- **Capture UI**: the player fills **one Band at a time**, not one team at a time — see `docs/adr/0008-predict-the-table-group-fill-capture.md` for the original shape and `docs/adr/0011-predict-the-table-capture-v2.md` for the current one, which supersedes 0008's two-grammar review mode and its over/under-fill rule. `docs/predict-table-problem.md` explains why the original per-team version was replaced. All 8 Bands are on screen throughout, in table order. There is no separate review mode: the board has zero or one **open Band** at a time, and tapping a club always means "put it in the open Band," whether that club comes from the roster, from another Band, or from the open Band itself (which toggle-reverts it to wherever it came from). Band headers toggle open/closed and are the only navigation — there is no rail and no permanent chevron-paging control. The open Band expands with the full 20-team roster inside it, directly beneath its members; the other Bands collapse to a row carrying their full membership, so with every Band closed the board already reads as the finished table — reached with the same gesture used to fill it. Teams show a 3-letter code, full name, and a real club-color kit-stripe for identity — no crest, per `docs/DESIGN_SYSTEM.md`'s no-crest trademark constraint. Multi-club Bands list their members alphabetically (order carries no scoring weight, and a vertical stack would otherwise read as a ranking); a one-club Band (Champion, Runners Up) puts its club on the header line instead. The roster is ordered by last season's finishing position, with that position shown ("Promoted" for a club new to the league); an already-placed club demotes below an "Already placed" caption rather than disappearing, so it stays tappable, and the demotion re-group runs only when the open Band changes, never on a tap, so nothing shifts under a player's finger mid-flow. **A Band can never exceed its target size.** Tapping a club into a full Band swaps it in for that Band's most-recently-added club (last in, first out), which returns to the roster unplaced — stated in advance (a line above the roster names the club, plus a warm tint on its card) and always undoable. This buys one invariant in place of eight counters: `20 placed` implies every Band is exactly its target size, which implies every Band Bonus is in play. Submitting with any Band still short still warns once and lets the player submit anyway, forfeiting that Band's bonus — over-filling itself is no longer possible. No drag-and-drop reordering exists anywhere in this feature. Every move persists immediately (not just on final submit), so the flow is safely resumable. **Band-by-Band guidance**: once the open Band reaches exactly its target size, a passive "Band _n_ of 8" position readout is visible throughout filling, and a "Next: [Band] →" prompt appears at the bottom of that Band — tapping it opens the next Band ahead (in table order) that's still under its target; the player can ignore it and open any Band manually instead. This is an advisory nudge, not a new navigation surface — it doesn't reach backward and never wraps. Once **every** Band is exactly filled that same prompt becomes **"Review my table"**, which closes the open Band and so shows the finished table — finishing is the same motion, in the same place, as advancing.
- **Table Bands** (by final league position): Champion (1), Runners Up (2), Champions League (3–5), Europe (6–8), Mid Table (9–11), Lower Table (12–14), Relegation Battle (15–17), Relegated (18–20).
- **Scoring** — three components summing to a maximum of **200**. See `docs/adr/0010-predict-the-table-scoring.md` for the placement-curve and Bold Call reasoning and the rejected alternatives, and `docs/adr/0011-predict-the-table-capture-v2.md` for the Band Bonus re-tuning that followed the Runners Up split (Champion and Champions League moved from 15 to 10 each to hold the 200 ceiling at eight Bands).
  ...
  Band Bonus (exact full membership, any order within)
    Relegated                                        15
    Champion / Runners Up / Champions League /
    Europe / Mid / Lower / Relegation Battle         10 each    max  85
                                                               -------
  MAXIMUM                                                          200
  ```

  `band_distance` is the number of Bands between the player's predicted Band and the team's actual Band (Bands ordered 1–8 as above). A team left unplaced scores nothing — the same as one placed 3+ Bands out. A Band whose predicted membership isn't exactly its actual membership forfeits its bonus. Relegated carries the one larger bonus, on the Band a season's bottom is actually about; it needs three teams exact, the same as any other multi-club Band, so the extra 5 points reward getting the bottom of the table right rather than an easier target.
