...
| `20260813010000_team_display_names.sql`                  | Adds `full_name` and `display_name` columns to `teams`. `full_name` preserves the provider label; `display_name` is the common UI label for Predict the Table filling; `name` is the page-facing suffix-cleaned short name.                                               |
| `20260813020000_table_prediction_deadline.sql`           | Adds `get_db_time()` function and three transactional RPCs (`table_prediction_assign`, `table_prediction_unassign`, `table_prediction_submit`) that enforce the `2026-08-31T14:00:00Z` deadline against DB time. Adds `idx_matches_season` index on `matches(season_id)`. |
