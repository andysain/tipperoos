...
- Every authenticated page calls `loadActivePlayer()` (`src/app/_lib/session-player.ts`) as its first `await`, before its own loaders — the server-side gate that redirects a logged-out visitor to `/login` and a forced-PIN-reset player to `/reset-pin`. Exceptions: `/login` and `/reset-pin` run their own lighter checks (calling the gate from `/reset-pin` would loop); `/admin/*` must stay a bodyless 404 for a stranger, so `src/app/admin/layout.tsx` applies `enforcePinResetGate()` (redirect only for a session already mid-reset) and each page keeps its own `requireAdmin()` → `notFound()`.
