...
function request(body: unknown) {
  return new Request("http://localhost/api/table-predictions/unassign", {
    method: "POST",
    headers: {
...
describe("POST /api/table-predictions/unassign", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("rejects an on-time edit at the fixed deadline", async () => {
    ...
    const response = await POST(request({ teamId: "team-1" }));
  ...
  it("saves an unassignment through the transactional RPC", async () => {
    const response = await POST(request({ teamId: "team-1" }));

    expect(response.status).toBe(200);
    expect(rpcMock).toHaveBeenCalledWith("table_prediction_unassign", {
