...
describe("deriveKeyHex (fixed-salt golden values)", () => {
  it("derives the expected key for secret 1234 / a known salt", async () => {
    const hex = await deriveKeyHex("1234", "00112233445566778899aabbccddeeff");
    expect(hex).toBe(
  ...
  it("derives the expected key for secret 0000 / a different salt", async () => {
    const hex = await deriveKeyHex("0000", "aabbccddeeff00112233445566778899");
    expect(hex).toBe(
  ...
  it("derives the expected key for secret 4321 / an all-zero salt", async () => {
    const hex = await deriveKeyHex("4321", "00000000000000000000000000000000");
    expect(hex).toBe(
  ...
  it("different secrets produce different keys for the same salt", async () => {
    const salt = "00112233445566778899aabbccddeeff";
    const a = await deriveKeyHex("1111", salt);
    const b = await deriveKeyHex("2222", salt);
    expect(a).not.toBe(b);
  });
});
