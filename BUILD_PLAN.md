...
15. **Bot pick-generation mechanics** *(gap from GAPS.md review, 2026-08-01)* — three bot types carry forward, ported from the old app's actual logic (`admin_ops.py`), not reinvented: **Random Bot** (independent random plausible scoreline per side), **1-1 Bot** (always 1–1), **Median Bot** (rounded median of that match's human picks, generated only *after* lock — a crowd reference, not a blind guess). **ELO bot is dropped**, consistent with the existing "no smart bots" decision. Your call, not a default — you explicitly chose to keep 3 of 4 old bot types rather than scoping bots out of v1 entirely.
16. **Late joiners** *(gap from GAPS.md review, 2026-08-01)* — a player who signs up after gameweek 1 has begun. Not eligible for the season "winner" title (didn't compete the full season, same exclusion mechanism as the admin). Can submit Predict the Table at any time after joining, or skip it — both optional for them, unlike the mandatory pre-season capture for on-time players. Gameweeks before joining score 0 with no special-case logic (no picks exist for those matches).
17. **Staging Supabase project** *(gap from GAPS.md review, 2026-08-01)* — a second free Supabase project is created alongside the production one, specifically so the week-3 dry run and general testing never touch the project that's about to go live for real players. Free tier allows 2 active projects per org; the old World Cup project (dormant since issue #3) is expected to auto-pause from inactivity around 2026-08-08, freeing headroom well before this is needed.
18. **football-data.org rate-limit sizing** *(gap from GAPS.md review, 2026-08-01, resolved by direct research rather than assumption)* — confirmed: the Premier League is included in the free tier, at a 10-calls/minute limit. A sync cycle every 10–15 minutes doing **one batched date-range call** stays far inside that budget — no risk, provided the sync is built as one call per cycle, not one call per fixture.
19. **Session cookie / CSRF** *(gap from GAPS.md review, 2026-08-01)* — `httpOnly` + `secure` + `sameSite=Lax` cookie, plus a custom-header check on all state-changing API routes. Sufficient for this threat model without a full CSRF-token library, since hand-rolled sessions don't get this for free the way a standard auth library would.
20. **Forgot-PIN reset** *(gap from GAPS.md review, 2026-08-01)* — admin-assisted reset only (admin resets it, player sets a new PIN on next login). Matches the existing "PIN security proportionate to actual stakes" stance — no self-service email-reset flow needed for a group this size.
21. **Three-environment mapping** *(gap surfaced 2026-08-01)* — local dev (`next dev`) and Vercel Preview deployments both point at the **staging** Supabase project; only the Vercel Production deployment (from `main`) points at **production**. Free on Vercel's Hobby tier, reuses the staging project from decision 17 rather than needing a 3rd. Required discipline: apply schema migrations to staging first, confirm, then production — before merging the branch that depends on it.
